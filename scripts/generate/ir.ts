    ...
    for (const [path, pathItem] of Object.entries(doc.paths ?? {}).sort()) {
        for (const method of HTTP_METHODS) {
            ...
            if (tags.length !== 1 || !tag) {
                fail(
                    `${where} (${operationId}) must have exactly one tag, got [${tags.join(', ')}]`,
                );
            }
            ...
            for (const param of params) {
                if (param.$ref)
                    fail(`${where} (${operationId}) uses a $ref parameter; unsupported`);
                if (param.in === 'header') {
            ...
            for (const name of templateParams) {
                if (!declaredPathParams.has(name)) {
                    fail(
                        `${where} (${operationId}) path template param "{${name}}" is not declared`,
                    );
                }
            }
            ...
            const successEntries = Object.entries(op.responses ?? {}).filter(([s]) =>
                s.startsWith('2'),
            );
