...
try {
  ...
  for (const spec of SPECS) {
    const doc = parse(await Bun.file(join(SPECS_DIR, `${spec.id}.yaml`)).text()) as SpecDocument;
  ...
  for (const model of models) {
    ...
    console.log(`  ${model.specId.padEnd(14)} ${String(count).padStart(3)} ops  (${namespaces})`);
  }
