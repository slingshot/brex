...
describe("assignNames", () => {
  test("assigns namespace and method from tag + operationId", () => {
    const named = assignNames([op({ operationId: "createVendor", tag: "Vendors" })], NO_OVERRIDES);
  ...
  test("tag overrides remap namespace and resource nouns", () => {
    ...
    const named = assignNames([op({ operationId: "listSpendBudgets", tag: "Budgets" })], overrides);
